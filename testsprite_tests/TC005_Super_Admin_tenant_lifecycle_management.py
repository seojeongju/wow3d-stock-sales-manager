...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Tenant creation failed due to permission error').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Super Admin tenant management test did not pass as expected. The test plan execution has failed, indicating issues with tenant creation, update, or deletion permissions or data validation.')
