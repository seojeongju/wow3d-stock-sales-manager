...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Unauthorized Tenant Access Detected').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test failed: Super Admin impersonation via X-Tenant-ID header is not strictly validated, allowing unauthorized cross-tenant access.')
