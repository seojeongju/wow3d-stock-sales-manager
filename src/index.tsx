...
<body class="h-screen w-full flex items-center justify-center relative">
    ...
    <div class="relative z-10 w-full max-w-md px-4">
        <div class="glass-panel rounded-3xl overflow-hidden p-8 animate-fade-in-up">
            ...
            <div class="text-center mb-10">
                <img src="/static/login_logo.png" alt="WOW Sales ERP" class="h-16 mx-auto mb-6 object-contain drop-shadow-lg">
                <p class="text-slate-400 text-sm font-medium tracking-wide">Enterprise Resource Planning</p>
            </div>

            <!-- Tabs -->
            <div class="flex p-1 bg-slate-800/50 rounded-xl mb-8">
                <button onclick="switchTab('login')" id="loginTabBtn" class="flex-1 py-2.5 text-sm font-bold rounded-lg bg-indigo-600 text-white shadow-lg transition-all">로그인</button>
                <button onclick="switchTab('register')" id="registerTabBtn" class="flex-1 py-2.5 text-sm font-medium rounded-lg text-slate-400 hover:text-white transition-all">회원가입</button>
            </div>
            ...
            <form id="loginForm" onsubmit="handleLogin(event)" class="space-y-5">
                <div class="space-y-1">
                    ...
                    <div class="relative">
                        ...
                        <input type="email" id="loginEmail" required class="w-full pl-11 pr-4 py-3 rounded-xl input-dark focus:outline-none transition-all" placeholder="name@company.com">
                    </div>
                </div>
                <div class="space-y-1">
                    ...
                    <div class="relative">
                        ...
                        <input type="password" id="loginPassword" required class="w-full pl-11 pr-4 py-3 rounded-xl input-dark focus:outline-none transition-all" placeholder="••••••••">
                    </div>
                </div>
                
                <button type="submit" class="w-full bg-gradient-to-r from-indigo-500 to-blue-600 text-white py-3.5 rounded-xl font-bold text-lg hover:from-indigo-600 hover:to-blue-700 transition-all shadow-lg shadow-indigo-500/30 mt-4">
                    Sign In
                </button>

                <div class="flex justify-between text-sm mt-6 px-1">
                    <button type="button" onclick="showFindEmailModal()" class="text-slate-500 hover:text-indigo-400 transition-colors">아이디 찾기</button>
                    <button type="button" onclick="showResetPasswordModal()" class="text-slate-500 hover:text-indigo-400 transition-colors">비밀번호 찾기</button>
                </div>
            </form>
            ...
            <form id="registerForm" onsubmit="handleRegister(event)" class="space-y-4 hidden">
                <div class="space-y-1">
                    ...
                    <div class="relative">
                        <i class="fas fa-envelope absolute left-4 top-3.5 text-slate-500"></i>
                        <input type="email" id="regEmail" required class="w-full pl-11 pr-4 py-3 rounded-xl input-dark focus:outline-none transition-all" placeholder="name@company.com">
                    </div>
                </div>
                <div class="space-y-1">
                    <label class="text-xs font-bold text-slate-400 ml-1">PASSWORD</label>
                    <div class="relative">
                        <i class="fas fa-lock absolute left-4 top-3.5 text-slate-500"></i>
                        <input type="password" id="regPassword" required class="w-full pl-11 pr-4 py-3 rounded-xl input-dark focus:outline-none transition-all" placeholder="••••••••">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-xs font-bold text-slate-400 ml-1">NAME</label>
                        <div class="relative">
                            <i class="fas fa-user absolute left-4 top-3.5 text-slate-500"></i>
                            <input type="text" id="regName" required class="w-full pl-11 pr-4 py-3 rounded-xl input-dark focus:outline-none transition-all" placeholder="이름">
                        </div>
                    </div>
                    <div class="space-y-1">
                        <label class="text-xs font-bold text-slate-400 ml-1">COMPANY</label>
                        <div class="relative">
                            <i class="fas fa-building absolute left-4 top-3.5 text-slate-500"></i>
                            <input type="text" id="regCompany" required class="w-full pl-11 pr-4 py-3 rounded-xl input-dark focus:outline-none transition-all" placeholder="회사명">
                        </div>
                    </div>
                </div>
                <button type="submit" class="w-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white py-3.5 rounded-xl font-bold text-lg hover:from-emerald-600 hover:to-teal-700 transition-all shadow-lg shadow-emerald-500/30 mt-4">
                    Create Account
                </button>
            </form>
        </div>
    ...
    <!-- Modals (Dark Theme) -->
    <div id="findEmailModal" class="hidden fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
        <div class="bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-xl font-bold text-white">아이디 찾기</h3>
                <button onclick="closeFindEmailModal()" class="text-slate-400 hover:text-white transition-colors">
                    <i class="fas fa-times text-lg"></i>
                </button>
            </div>
            <form onsubmit="handleFindEmail(event)" class="space-y-4">
                <div>
                    <label class="block text-sm font-bold text-slate-400 mb-1">이름</label>
                    <input type="text" id="findEmailName" required class="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white focus:outline-none focus:border-indigo-500" placeholder="가입 시 등록한 이름">
                </div>
                <button type="submit" class="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700 transition-colors">
                    찾기
                </button>
            </form>
            <div id="findEmailResult" class="hidden mt-4 p-4 bg-emerald-900/30 border border-emerald-800 rounded-xl">
                <p class="text-sm text-emerald-400 font-bold mb-1"><i class="fas fa-check-circle mr-2"></i>아이디를 찾았습니다!</p>
                <p class="text-sm text-slate-300 pl-6" id="foundEmailMessage"></p>
            </div>
        </div>
    </div>

    <div id="resetPasswordModal" class="hidden fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
        <div class="bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-xl font-bold text-white">비밀번호 재설정</h3>
                <button onclick="closeResetPasswordModal()" class="text-slate-400 hover:text-white transition-colors">
                    <i class="fas fa-times text-lg"></i>
                </button>
            </div>
            <form onsubmit="handleResetPassword(event)" class="space-y-4">
                <div>
                    <label class="block text-sm font-bold text-slate-400 mb-1">이메일</label>
                    <input type="email" id="resetEmail" required class="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white focus:outline-none focus:border-indigo-500" placeholder="example@company.com">
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-400 mb-1">이름</label>
                    <input type="text" id="resetName" required class="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white focus:outline-none focus:border-indigo-500" placeholder="홍길동">
                </div>
                <button type="submit" class="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700 transition-colors">
                    임시 비밀번호 발급
                </button>
            </form>
            <div id="resetPasswordResult" class="hidden mt-4 p-4 bg-emerald-900/30 border border-emerald-800 rounded-xl">
                <p class="text-sm text-emerald-400 font-bold mb-2" id="resetSuccessMessage"></p>
                <p class="text-sm text-slate-300 mb-2" id="tempPasswordDisplay" style="display: none;">임시 비밀번호: <span class="font-mono font-bold text-indigo-400 bg-indigo-900/50 px-2 py-1 rounded" id="tempPassword"></span></p>
            </div>
        </div>
    </div>
