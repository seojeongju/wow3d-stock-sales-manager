...
app.post('/', async (c) => {
  ...
  const result = await DB.prepare(`
    INSERT INTO customers (name, phone, email, zip_code, address, address_detail, company, department, position, birthday, purchase_path, business_number, notes, grade, tenant_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    body.name,
    body.phone,
...
app.put('/:id', async (c) => {
  ...
  if (body.business_number !== undefined) {
    ...
    params.push(body.business_number)
  }
